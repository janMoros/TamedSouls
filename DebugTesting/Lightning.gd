...
var cooldown = false
export var is_outside = false

func _physics_process(delta):
	if OS.is_debug_build() and Input.is_action_pressed("debug_action") and !cooldown:
		var target = get_local_mouse_position()
		zap(target)
		#if Global.lightning_target != null:
		#	var target = Global.lightning_target.global_position
		#	global_zap(target)
			

func global_zap(global_target, yOffset=0):
	var target = to_local(global_target)
	target.y -= yOffset
	zap(target)
